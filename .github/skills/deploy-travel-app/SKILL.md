...
## When to Use
- After editing the trip platform frontend under `app/` and wanting it live.
...
- Target backend Function App is derived from `app/js/config.js` `API_BASE`
...
- [ ] `GET /api/trips/yunnan2026` returns JSON after backend deployment.
...
- If you changed the Function App name, update `API_BASE` in `app/js/config.js`, then deploy frontend too.
